    ...
    private int spawnFernPatches(CommandSourceStack source, String type, int radius, int attempts) {
        try {
            ...
            if (type.equalsIgnoreCase("ice")) {
                feature = registry.getHolderOrThrow(ModConfiguredFeatures.ICE_FERN_CONFIG_KEY);
            } else if (type.equalsIgnoreCase("sun")) {
                feature = registry.getHolderOrThrow(ModConfiguredFeatures.SUN_FERN_CONFIG_KEY);
            } else {
