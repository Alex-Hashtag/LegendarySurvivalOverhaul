...
public class WetnessCapability implements IWetnessCapability {
    ...
    /**
     * This probably isn't too terribly performance friendly but it's something at least
     *
     * TODO: optimization!!
     */
    @Override
    public void tickUpdate(Player player, Level level, Phase phase) {
        ...
        if (player.getVehicle() != null) {
            // Use BuiltInRegistries for direct getKey lookups
        ...
        if (fluidState.isEmpty() && !blockState.is(Blocks.WATER_CAULDRON) && !MeadowUtil.isInWoodenWaterCauldron(blockState)) {
            ...
            if (fluid instanceof BaseFlowingFluid) {
                BaseFlowingFluid flowing = (BaseFlowingFluid) fluidState.getType();

                if (this.wetness > 0 && flowing.getFluidType().isAir()) {
                    this.addWetness(Config.Baked.wetnessDecrease);
                    return;
                }

                int temperature = flowing.getFluidType().getTemperature();
